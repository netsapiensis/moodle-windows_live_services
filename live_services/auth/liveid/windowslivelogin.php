...
class WindowsLiveLogin
{
    ...
    public function getSecret()
    {
        if(!$this->_secret)
        {
            self::fatal("Error: getSecret: Secret key is expected to be non-null and longer than 16 characters.");
        }
        return $this->_secret;
    }
    private $_oldsignkey;
 ...
 */
    ...
    {
        $url  = self::getBaseUrl(); 
        $url .= 'authorize?client_id=' . self::getAppId();
        $url .= '&scope=' . self::getScope();
        $url .= '&response_type=' . self::getResponseType();
        $url .= '&redirect_uri=' . self::getReturnUrl();
        return $url;
    }
    ...
    {
        if (!$this->_scope) {
            self::fatal('Error: getScope: Scope was not set. Aborting.');
        }
        return $this->_scope;
    }
    ...
    {
        if (!$scope) {
            self::fatal('Error: setScope: Null Scope.');
        }
        $this->_scope = $scope;
    }
    ...
    public function getResponseType()
    {
        ...
        {
            self::fatal('Error: getResponseType: ResponseType was not set. Aborting.');
        }
    ...
    public function setResponseType($responseType)
    {
        if(!$responseType)
        {
            self::fatal('Error: setResponseType: Null ResponseType.');
        }
    ...
    public function ExpireCookies()
    {
        ...
        setcookie ("c_error_description", "", time() - 3600,'/');
        setcookie ("c_accessToken", "", time() - 3600,'/');
        setcookie ("c_expiry", "", time() - 3600,'/');
        setcookie ("c_code", "", time() - 3600,'/');
        setcookie ("c_refreshToken", "", time() - 3600,'/');
     ...
     */
    public function setAuthCookies($cookies)
    {
        foreach($cookies as $key => $value)
        {
            setcookie ($key, $value, time() + 36000,'/');
        }
        setcookie ('c_clientId', self::getAppId(), time() + 36000,'/'); //clientID == appId
     ...
     */
    public function convertParamsToCookies($array)
    {
        $cookies = array();
        ...
        foreach(array_keys($array) as $getParam)
        {
            ...
            {
                case 'scope': //scope
                    $cookies['c_scope'] = str_replace(';', ',',$array['scope']);
                    break;
                case 'error':
                    $cookies['c_error'] = ' ' . $array['error'];
                    break;
                case 'error_description':
                    $cookies['c_error_description'] = ' ' . $array['error_description'];
                    break;
                case 'access_token':
                    $cookies['c_accessToken']= $array['access_token'];
                    break;
                case 'expires_in':
                    $cookies['c_expiry']= date('j/m/Y g:i:s A', $array['expires_in']);
                    break;
                case 'code':
                    $cookies['c_code']= $array['code'];
                    break;
                case 'refresh_token':
                    $cookies['c_refreshToken']= $array['refresh_token'];
                    break;
            }
        }
        return $cookies;
...
/**
     ...
     */
    public function getAuthorizationToken($authUrl, $appId, $appSecret, $callbackUrl, $verificationCode)
    {
        //Using the returned verification code build a query to the
        //authorization url that will return the authorized token.
        $tokenRequest = 'client_id=' . urlencode($appId)
     ...
     */
    public function postWRAPRequest($posturl, $postvars)
    {
     ...
     */
    public function parseWRAPResponse($response)
    {
        //Firslty remove any extraneous header information from the returned POST variables
        $pos = strpos($response, 'access_token');
        if ($pos === false)
        {
            $pos = strpos($response, 'error_description');
        }
        //$codes = '?' . substr($response, $pos, strlen($response));
        $codes ='?' . substr($response, $pos-1, strlen($response));
         ...
         $newcode=preg_replace($patterns, $replacements, $codes);

        //RegEx the string to seperate out the variables and thier values
        if (preg_match_all('/[?&]([^&=]+)=([^&=]+)/', $newcode, $matches))
        {
    ...
    public function getRefreshedToken($refreshToken) {
        ...
        $BaseRefreshTokenUrl='https://oauth.live.com/token';
        $tokenRequest = 'client_id=' . urlencode(self::getAppId())
                . '&redirect_uri=' . urlencode(self::getReturnUrl())
                . '&wrap_client_secret=' . urlencode(self::getSecret())
                . '&refresh_token=' . urlencode($refreshToken)
                . 'grant_type=refresh_token';

        $response = $this->postWRAPRequest($BaseRefreshTokenUrl, $tokenRequest);
        $refresh_params=$this->parsePOSTResponse($response);
        $cookies_auth = $this->convertParamsToCookies($refresh_params);
        $cookies = array_merge($cookies_setup, $cookies_auth);
        $this->setAuthCookies($cookies);
    }
